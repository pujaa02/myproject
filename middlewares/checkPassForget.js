...
exports.checkPassForget = async (req, res, next) => {
  ...
  let query1 = `SELECT access_key,modified_at,pass_updated FROM users WHERE user_id=?`;
 
  let result = await execute(query1, [userId]);
  if (
    checkLinkTime(result[0].modified_at) == false ||
    result[0].pass_updated == "1"
  ) {
